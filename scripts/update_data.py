...
# =====================================================================
# 1. SCRAPING STEAM API E ARPG TIMELINE API (CON PARACADUTE)
# =====================================================================
def get_season_timer(game_key, fallback_name):
    # 1. TENTATIVO VIA API UFFICIALE
    mapping = {
        "poe1": "path-of-exile",
        "poe2": "path-of-exile2",
        "d4": "diablo-iv",
    ...
    if slug:
        try:
            # Interrogazione API (Endpoint generico eventi/stagioni)
            url = "https://arpg-timeline.com/api/events"
            headers = {'User-Agent': 'Mozilla/5.0', 'Accept': 'application/json'}
            res = requests.get(url, headers=headers, timeout=8)
            
            if res.status_code == 200:
                events = res.json()
                # Cerca l'evento attivo per il gioco specifico
                for event in events:
                    if event.get('gameSlug') == slug and event.get('isActive'):
                        season_name = event.get('name', fallback_name)
                        end_date_str = event.get('endDate')
                        
                        if end_date_str:
                            end_date = datetime.strptime(end_date_str[:10], "%Y-%m-%d")
                            days_left = (end_date - datetime.utcnow()).days
                            time_left = f" (Termina tra {days_left}g)" if days_left > 0 else " (In conclusione)"
                            return f"{season_name}{time_left}"
                        return season_name
        except Exception:
            pass # Se l'API fallisce, passa al piano B senza bloccare il sito

    # 2. PIANO B: CALENDARIO INTERNO INFALLIBILE
    target_dates = {
        "poe1": datetime(2026, 11, 28),
        "poe2": datetime(2026, 12, 11),
        "d4": datetime(2026, 10, 20),
        "le": datetime(2026, 10, 15),
        "d2": datetime(2026, 11, 10)
    }
    
    time_left_str = ""
    if game_key in target_dates:
        days_left = (target_dates[game_key] - datetime.utcnow()).days
        if days_left > 0:
            time_left_str = f" (Termina tra {days_left}g)"
        else:
            time_left_str = " (In conclusione)"
            
    return f"{fallback_name}{time_left_str}"
...
def main():
    ...
    print("Recupero dati 15 ARPG in tempo reale da Steam e API...")
    steam_games = {
    ...
    seasons = {
        "poe1": get_season_timer("poe1", "Lega 3.29 Curse of the Allflame"),
        "poe2": get_season_timer("poe2", "0.5.5 - The Forbidden Rites Event"),
        "d4": get_season_timer("d4", "Stagione 7"),
        "le": get_season_timer("le", "Season 4"),
        "d2": get_season_timer("d2", "Ladder Stagione 14")
    }
    ...
    for game, season in seasons.items():
        patches_db["games"][game] = {
            "currentPatch": season,
            "changes": [f"Dati sincronizzati tramite API il {today_str}.", "Consulta il sito ufficiale per i dettagli di bilanciamento."],
    ...
    print("[+] File JSON API aggiornati con successo.")
